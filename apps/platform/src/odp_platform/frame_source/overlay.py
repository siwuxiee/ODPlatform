...
帧率口径(写论文/快照都用得上):
  - capture(捕获) : 帧源读帧的速率(相机/解码端能多快给帧)
  - infer(推理)   : 模型推理速率, 取自 ultralytics result.speed(纯模型, 不含读帧/绘制)
  - render(渲染)  : 美化绘制(BeautifyVisualizer.draw)的速率
  - loop(端到端)  : 整个主循环一帧的速率(读+推理+绘制+显示), 即实际吞吐
  - current(当前) : loop 的瞬时值(最近一帧的 1/耗时), 反映实时波动
"""
...
class FPSCounter:
    """滑动窗口 FPS(按每帧耗时 ms 估算). 用于"单级能跑多快"口径(capture/infer/render)."""
...
class RateCounter:
    """按 (帧数, 耗时秒) 样本滚动统计吞吐 FPS = Σ帧 / Σ耗时.

    ★ 批处理感知: 一批算一个样本, 例如 (16, 0.09s). 这样不会被"批内逐帧派发只差几
    微秒"污染 —— 之前 当前FPS=50万 就是按单帧间隔算 1/Δt 炸出来的.
    窗口小→对最近几帧敏感(当前FPS); 窗口大→平滑(平均loop吞吐).
    """
    ...
    def fps(self) -> float:
        if not self._samples:
            return 0.0
        tot_f = sum(f for f, _ in self._samples)
        tot_t = sum(t for _, t in self._samples)
        return tot_f / tot_t if tot_t > 0 else 0.0


@dataclass
class Metrics:
    """一次推理的多维帧率 + 模型 speed 分解."""
    capture: FPSCounter = field(default_factory=FPSCounter)       # 单级: 读帧速率
    infer:   FPSCounter = field(default_factory=FPSCounter)        # 单级: 模型推理速率(result.speed)
    render:  FPSCounter = field(default_factory=FPSCounter)        # 单级: 绘制速率
    loop:    RateCounter = field(default_factory=lambda: RateCounter(60))   # 端到端吞吐(平滑)
    current: RateCounter = field(default_factory=lambda: RateCounter(8))    # 端到端吞吐(最近几帧)
...
def draw_hud(
    ...
    *,
    ...
    """左上角半透明信息面板. show_info=False 则只画一个极简 REC/帧率行(或什么都不画).
