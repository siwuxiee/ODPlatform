...
"""推理服务编排器 (D9: 接受 sink/hooks/cancel_token 接缝).

★ 核心纪律 (跟 D6 TrainService 完全同构): 不重新发明 D5 / 帧源 / 美化已有的轮子.

★ D9 改造 (向后 100% 兼容):
  - predict() 新增 3 个 keyword-only 参数: output_sink / hooks / cancel_token
  - 都默认 None, 内部根据 want_save 决定默认 sink (LocalFileSink / NullSink)
  - 不传 = 完全等价于改造前的 CLI 行为
  - 删除了原 _ResultWriter 类 (功能搬到 sinks.LocalFileSink)
  - 顶层 try/except 也触发 hooks.fire_complete / fire_error
...
from odp_platform.frame_source import create_frame_source, SourceType, IMAGE_EXTENSIONS
from odp_platform.visualization import BeautifyVisualizer, DrawStyle

from .cancel import CancelToken                              # ★ D9
from .hooks import InferHooks                                # ★ D9
from .pipeline_config import PipelineConfig, load_pipeline_config
from .sinks import LocalFileSink, NullSink, OutputSink       # ★ D9

logger = logging.getLogger(__name__)


# ============================================================================
# 逐帧 model() 的 predict 计算参数白名单 (不变)
# ============================================================================
...
# ============================================================================
# ★ D9: 删除了原 _ResultWriter 类
# 功能完全搬到 sinks.LocalFileSink, 行为 100% 等价 (CLI 用户感觉不到变化).
# ============================================================================
...
class InferService:
    ...
    def predict(
        ...
        *,
        ...
        show_info: bool = True,
        # ---- ★ D9: 新增 3 个 keyword-only 参数, 默认 None 让 CLI 行为不变 ----
        ...
        """
        # ★ D9: hooks 始终至少是个空 InferHooks 实例 (内部 fire 时 short-circuit, 零开销)
        if hooks is None:
            hooks = InferHooks()
        ...
        try:
            ...
            # ================================================================
            # ★ D9: 决定最终 sink
            #   - 调用方传了 sink → 用调用方的 (此时 want_save 仅影响 pipeline
            #     的"渲染队列丢旧 vs 阻塞"策略, 不影响 sink 选择)
            #   - 调用方没传 + want_save → LocalFileSink (CLI 默认, 等价原 _ResultWriter)
            #   - 调用方没传 + 不 save  → NullSink     (CLI --no-save, 等价原 writer=None)
            # ================================================================
            if output_sink is None:
                output_sink = LocalFileSink() if want_save else NullSink()
                _sink_owned = True
            else:
                _sink_owned = False    # 调用方传的, 调用方负责清理 (但 pipeline 还是会调 close)
            ...
            # ★ D9: 触发 on_complete (没注册 → fire 内部 short-circuit, 零开销)
...
# ============================================================================
# 帧处理器 (不变 — 是 D8/D9 接缝, 留给 TensorRT/ONNX 后端扩展用)
# ============================================================================
@dataclass
